...
- Copy `.env.example` to `.env` and fill in the values:
  ...
  - `TWITCH_CLIENT_ID` and `TWITCH_CLIENT_SECRET` - from the [Twitch Developer Console](https://dev.twitch.tv/console).
  - `IGDB_PROXY_KEY` - shared secret for the IGDB search Netlify function.
- Run `pnpm install`.
- Run `pnpm start`.
